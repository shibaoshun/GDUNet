...
class Phantom2dDatasettest():
    ...
    def radon_transform_120(self, num_view=120):
        ...
        geometry = odl.tomo.FanBeamGeometry(angle_partition, detector_partition, src_radius=197.246969,
                                            det_radius=220.798)
    ...
    def __getitem__(self, ii):
        ...
        if 'npy' in self.sp_files[ii]:
            ...
            # 使用numpy读取数据
            sino_data = np.fromfile(filename, dtype=np.float32)
            sino_data = sino_data.reshape((height, width))
            sino = sino_data

            # sino = torch.from_numpy(sino)
            # sino = sino.unsqueeze(0)

            # zero_rows_count = np.sum(np.all(sino == 0, axis=0))
            #
            # # 计算行数
            # total_rows = sino.shape[0]
            #
            # # 判断条件是否成立
            ...
            # if self.args.mode == 'sparse':
            #     nx, ny = np.array(360), np.array(self.args.sino_size[1]).astype(int)
            # noise = noise_std * np.random.randn(nx, ny)
            # noise = torch.from_numpy(noise)
            # sino_noisy = sino_noisy + noise  # Ssv120
            ...
            sino[ 5:365:6, :] = 0
            sino_noisy_360 = sino
            sino_noisy_angle = sino_noisy_360[0:360:6, :]
            # sino_noisy_360 = np.expand_dims(sino_noisy_360, axis=0)
            # sino_noisy_360 = torch.from_numpy(sino_noisy_360)
            # sino_noisy_360 = sino_noisy_360.unsqueeze(0)
            # sino_noisy_360=sino_noisy_360.detach().numpy()

            sino_noisy_360 = torch.from_numpy(sino_noisy_360)
            sino_noisy_360 = sino_noisy_360.unsqueeze(0)
            # sino_noisy_360 = sino_noisy_360.numpy()

            fbpu = self.fbp(sino_noisy_360)
            ...
            # sino_gt = torch.from_numpy(sino_gt)
            # sino_gt = sino_gt.unsqueeze(0)
            # sino_gt = sino_gt.type(torch.FloatTensor)
            #
            #
...
class Phantom2dDatasetval1():
    ...
    def __getitem__(self, ii):
        ...
        return phantom, fbpu, sino_noisy, imgSAM, angle,img_1024_tensor,sino_gt
